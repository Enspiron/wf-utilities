...
export async function generateMetadata({ params }: ItemDetailRouteProps): Promise<Metadata> {
  const { id } = await params;
  const detail = await getItemDetailData('item', id);
  if (!detail) {
    return {
      title: 'Item Not Found - World Flipper Tools',
      description: 'The requested item could not be found.',
    };
  }

  const imageUrl = detail.imageCandidates[0];
  const canonicalPath = `/item/${encodeURIComponent(detail.entry.id)}`;
  const canonicalUrl = new URL(canonicalPath, getSiteBaseUrl()).toString();
  const title = `${detail.entry.name} - Item`;
  const description = (detail.entry.description || detail.entry.flavorText || 'World Flipper item detail.')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 180);
...
export default async function ItemDetailRoute({ params }: ItemDetailRouteProps) {
  ...
  return <ItemDetailPage routeKind='item' detail={detail} />;
}
